namespace Test.Client
{
    ...
    class Program
    {
        ...
        {
            _Bucket = Inputty.GetString("Bucket:", "s3-proxy-bucket1", false);
        }
        ...
        {
            _AccessKey = Inputty.GetString("Access key:", "YOUR_ACCESS_KEY_HERE", false);
        }

        static void SetSecretKey()
        {
            _SecretKey = Inputty.GetString("Secret key:", "YOUR_SECRET_KEY_HERE", false);
        }

        static void InitS3Client()
        {
            _S3Credentials = new Amazon.Runtime.BasicAWSCredentials(_AccessKey, _SecretKey);
            Console.WriteLine("Using credentials AccessKey=" + _AccessKey + " SecretKey=" + _SecretKey);
            Console.WriteLine("Using region: " + RegionEndpoint.USWest1.ToString());
            Console.WriteLine("Using endpoint: " + _Endpoint);
            Console.WriteLine("Using SSL: " + _Ssl.ToString());
            Console.WriteLine("Using signature version: " + _SignatureVersion);


            // 🧠 Tắt chunked signing qua môi trường
            Environment.SetEnvironmentVariable("AWS_S3_USE_CHUNK_ENCODING", "false");
            // 🧠 Bật full-payload signing (tắt streaming)
            Environment.SetEnvironmentVariable("AWS_S3_DISABLE_PAYLOAD_SIGNING", "false");
            // 🔧 Tắt chunk encoding và streaming payload signing để ký full body
            Environment.SetEnvironmentVariable("AWS_S3_USE_CHUNK_ENCODING", "false");
            Environment.SetEnvironmentVariable("AWS_S3_DISABLE_PAYLOAD_SIGNING", "false");
            ...
            _S3Config = new AmazonS3Config
            {
                ...
                UseHttp = !_Ssl, // Sửa: SSL = true -> UseHttp = false
                SignatureVersion = "4",
                // DisablePayloadSigning = false,  // ✅ Bắt buộc ký full body
                // UseChunkEncoding = false,       // ✅ Tắt chunk streaming
            };
        ...
        static void ListBucket()
        {
            ...
            {
                Console.WriteLine($"Unable to list bucket: {ex.Message}");
            }
        ...
        static void WriteBucket()
        {
            ...
            try
            {
                ...
                var response = _S3Client.PutBucketAsync(request).GetAwaiter().GetResult();

                if (response.HttpStatusCode == System.Net.HttpStatusCode.OK)
                    Console.WriteLine("✅ Bucket created successfully.");
                else
                    Console.WriteLine($"⚠️ Failed with status: {response.HttpStatusCode}");
            }
        ...
        string ComputeSHA256Hex(string filePath)
        {
        ...
        static void WriteObject()
        {
            ...
            if (!String.IsNullOrEmpty(filePath))
                bytes = File.ReadAllBytes(filePath); // ✅ Đọc toàn bộ vào RAM

            try
            {
                using var stream = new MemoryStream(bytes); // ✅ Stream có Length và Seek
                    ...
                    Headers =
            {
                ...
                string payloadHash = BitConverter.ToString(hashBytes).Replace("-", "").ToLowerInvariant();

                // ✅ Bắt buộc ký full-body
                request.Headers["x-amz-content-sha256"] = payloadHash;

                var response = _S3Client.PutObjectAsync(request).Result;

                Console.WriteLine($"Upload done: {response.HttpStatusCode}");
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
                ...
                using (StreamReader reader = new StreamReader(responseStream))
                {
                    if (response.ContentLength > 0)
                    {
                        ...
                        if (data != null && data.Length > 0)
                        {
                            ...
                            // save to file
                            string outFile = Path.Combine(Environment.CurrentDirectory, "downloaded_" + Path.GetFileName(id));
        ...
        static void ReadObjectTags()
        {
            ...
            try
            {
                ...
                request.Key = id;
                request.VersionId = ver.ToString();
                Console.WriteLine("Requesting tags for object " + id + " version " + ver.ToString());
                Console.WriteLine("Request: Bucket=" + request.BucketName + " Key=" + request.Key + " Version=" + request.VersionId);
        ...
        static void ObjectExists()
        {
            ...
            try
            {
                ...
                Console.WriteLine($"StorageClass: {response.StorageClass}");

                // Log custom metadata
                Console.WriteLine("User Metadata:");
                foreach (var kv in response.Metadata.Keys)
                {
                    Console.WriteLine($"  {kv}: {response.Metadata[kv]}");
                }
